...
class IRSSTile(Schema):
    ...
    view_template = schema.Choice(title=_(u"Display Class"),
                              source=_(u"Available RSS View Classes"),
                              required=True)
...
class RSSTile(Tile):
    ...
    def __call__(self):
        self.update()
        name = self.view_template or 'defaultrsstile'
...
class RSSFeed(object):
    ...
    def _get_image(self,summary):
        for image in html.fromstring(summary).findall(".//img"):
            ...
            for item in d['items']:
                try:
                    ...
                    itemdict = {
                        ...
                        'image': self._get_image(item.get('description', '')),
                    }
